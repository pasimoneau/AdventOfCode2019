...
public class Advent6
{
    ...
    public static void main(String[] args)
    {
        ...
        while (it.hasNext()) 
        {
            String orbiter = (String) it.next();
            String orbitee = (String) map.get(orbiter);

            while(true)
            {
                orbits++;
                if(orbitee.equals(CenterOfMass))
                    break;
                orbitee = (String) map.get(orbitee);
            }           
        }
        ...
        List<String> youOrbits = new ArrayList<String>();
        String youOrbitee = map.get(You);
        while(true)
        {
            youOrbits.add(youOrbitee);
            if(youOrbitee.equals(CenterOfMass))
                break;
            youOrbitee = (String) map.get(youOrbitee);
        }           

        System.out.println("YOU orbits:" + youOrbits);

        // ===============================================

        List<String> sanOrbits = new ArrayList<String>();
        String sanOrbitee = map.get(Santa);
        while(true)
        {
            sanOrbits.add(sanOrbitee);
            if(sanOrbitee.equals(CenterOfMass))
                break;
            sanOrbitee = (String) map.get(sanOrbitee);
        }           

        System.out.println("SAN orbits:" + sanOrbits);

        // ===============================================

        for(String youOrbit : youOrbits)
        {
            if(sanOrbits.contains(youOrbit))
            {
                int youHops = youOrbits.indexOf(youOrbit);
                int sanHops = sanOrbits.indexOf(youOrbit);

                System.out.println("Found match: YOU:" + youOrbit + ", hops : " + (youHops+sanHops));
            }
        }
    }
}
