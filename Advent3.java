    ...
    public static void main(String[] args)
    {
        ...
        for(String str : line1_tokenized)
        {
            ...
            for(int i=0; i<len; i++)
            {
                ...
                List<String> lines = (List<String>) map.get(key);
                if(lines==null || !lines.contains(lineName))
                    map.put(key, lineName);
            }
        }

        List<String> line2points = new ArrayList<String>();
        lineName = "2";
        posX = 0;
        posY = 0;
        for(String str : line2_tokenized)
        {
            String dir = str.substring(0, 1);
            int len    = Integer.parseInt(str.substring(1, str.length()));
            //
        ...
        while (it.hasNext()) {
            ...
            if(list.size()>1) 
            {
                ...
                // day3, part2
                int line1distance = line1points.indexOf(key) + 1;
                int line2distance = line2points.indexOf(key) + 1;
                int totalDistance = line1distance + line2distance;
                if(totalDistance < minimum_signal_length)
                    minimum_signal_length = totalDistance;
            }    
        }

        System.out.println("manhattan distance  : " + manhattan_distance);
        System.out.println("min signal distance : " + minimum_signal_length);
    }
}
